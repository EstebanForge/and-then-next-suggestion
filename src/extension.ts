...
export function activate(context: vscode.ExtensionContext) {
    ...
    statusBarItem.text = '$(zap) And Then Next Suggestion';
    ...
    const provider: vscode.InlineCompletionItemProvider = {
        async provideInlineCompletionItems(document, position, contextProv, token) {
            ...
            statusBarItem.text = `$(sync~spin) [${activeProfile.name}]`;
            activeRequests++;
            ...
            try {
                ...
                statusBarItem.text = `$(error) [${activeProfile.name}]`;
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('andThenNextSuggestion.switchProvider', async () => {
            ...
            items.push({ label: '$(output) Show Extension Logs', description: '', detail: '', profileId: 'logs' } as any);
            items.push({ label: '$(gear) Configure Profiles...', description: '', detail: '', profileId: 'settings' } as any);
